...
subprocess.run(['uptime'], stdout=(evidence / 'managed-before.txt').open('w'), check=True)
jobs = []
for i, cpu in enumerate(cores, 1):
    for name in ['finalizer_trigger', 'segmented_array_managed', 'phase_entry_trigger']:
        out = root / 'managed' / f'{name}-{i}'
...
while any(p.poll() is None for p, *_ in jobs):
    for pd in Path('/proc').iterdir():
        ...
        try:
            ...
            if root / 'managed' not in exe.parents:
                continue
            out = exe.parent
...
for proc, out, log, start, argv in jobs:
    ...
    result = dict(out=str(out), argv=argv, rc=rc, wall=time.monotonic()-start,
                  ...
                  loadfc_count=len(observations), loadfc=observations)
    (out / 'result.json').write_text(json.dumps(result, indent=2) + '\n')
    results.append(result)
subprocess.run(['uptime'], stdout=(evidence / 'managed-after.txt').open('w'), check=True)
(evidence / 'managed-results.json').write_text(json.dumps(results, indent=2) + '\n')
for r in results:
