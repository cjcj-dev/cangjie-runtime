...
if awk '/TryUntagRefField encounters invalid/,/^bool WCollector::|^}$/' \
      runtime/src/Heap/Collector/Relocate.cpp | grep -q 'GetAndTryTagRefField(target)'; then
  pass "① TryUntag write-back uses GetAndTryTagRefField"
else
  bad "① TryUntag write-back is not coloured"
fi
if grep -n 'RefField<> newRef(target)' runtime/src/Heap/Collector/Relocate.cpp | grep -q .; then
...
for t in 'const RefField<>&' 'const RootSlot&'; do
...
interior_cas=$(grep -c 'CasInstallInteriorPlain' runtime/src/Heap/Collector/Relocate.cpp || true)
...
if grep -n 'FixMinorEvacuatedSlot(RefField' -A80 runtime/src/Heap/Collector/Relocate.cpp \
...
# ④ the deleted census shell has no residue; the live write assertion owns the invariant
if grep -rn 'PlainCensus\|RunPlainCensus\|InjectPlainHeapWriteOnce\|NotePlainHeapWrite' \
    runtime/src runtime/tests >/dev/null 2>&1; then
  bad "④ deleted PlainCensus contract/call residue remains"
else
  pass "④ deleted PlainCensus has no source/test residue"
fi
if grep -n 'AssertColouredWriteIfEnabled' runtime/src/Common/BaseObject.cpp | grep -q .; then
  pass "④ live coloured-write assertion owns the invariant"
else
  bad "④ missing coloured-write assertion"
fi
