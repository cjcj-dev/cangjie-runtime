...
scratch=Path('/root/cj_build/agent_scratch/sym_cangjie_runtime_606_implement_r5674249495/final2')
...
(wt/'delivery-r2/final2').mkdir(exist_ok=True)
s=original[paths[1]]
needle='        VisitMinorRoots([this, &workStack, &currentMinorRoots](BaseObject* object) {'
assert s.count(needle)==1
changes['cut-producer']={paths[1]:s.replace(needle,'        [](auto&&, auto&&, uint64_t) {}([this, &workStack, &currentMinorRoots](BaseObject* object) {')}
changes['cut-combined']={**changes['cut-producer'],**changes['cut-consumer']}
for arm,files in changes.items():
 ...
 (wt/'delivery-r2/final2'/f'{arm}.diff').write_text(patch)
