PROGRESS=DONE · verdict=R1/R2与主线接线修正完成，送独立Review；保留已知红 ｜尺=产品SO单元＋P1阶段四臂 N=每单元臂1/每阶段臂3 · LANE=sym_cangjie_runtime_606_implement_r5674249495
DELIVERY_REF=cangjie-runtime|sym/606-implement-r5673376405|9de9226b8d58ed655534b195df2f6938be789062
SIDE_EFFECT: 仅候选分支与本棒产物；复用PR #628；不授予带已知红的合并许可
ROLE=implement
EVIDENCE=local:/root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/delivery-r2,kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2
...
本报告记录实现与验证，不代替独立Review。产品/测试源坐标为 `9de9226b8d58ed655534b195df2f6938be789062`；交付文档提交若在其后，runtime内容必须相同。正式交付头以 `/root/cj_build/reports/REPORT-sym_cangjie_runtime_606_implement_r5674249495.md` 为准。

初读指定仓 `/root/cj_build/cangjie_runtime` 的 cjcjdev/main，rc=0，得 `c3973505c171aa7e72095c3357ffd57f56156707`；任务表的 c9e91240 是上轮候选。advisor 033048Z已纠正。返工原分支，合入 #596 后基线为 `fb7d5282867aaa3b9d8b5df2f6691227ff3424ce`；merge提交 c46468bc0，冲突仅zMark.cpp的include，保留Generation和Barrier两侧。最后fetch/merge各rc=0、Already up to date，见 `final-main-sync.json`。三点diff、cut检查均用新基线，不用祖先关系猜内容。

裁决：`/root/cj_build/ops/advisor/outbox/sym_cangjie_runtime_606_implement_r5674249495-20260915T033048Z.md` 要求常态运行；033239Z限定类归属/夹具；035006Z批准selection时机修正；040345Z批准P1专用托管runner与#629装置归属；041513Z按Q59-B分开P1目标断言和#607后续字段故障。没有observer Abort、提前退出、删CHECK或放宽输入资格。

## ZGC对应与producer→consumer顺序

C=`/root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/runtime/src/Heap/z/`；Z=`/root/cj_build/reference/jdk/src/hotspot/share/gc/z/`。下表短锚按这两个根展开。初始顺序表/机械检索在 `order.md`、`producer-consumers.txt`；最终全量命中含命令/rc/原文在 `producer-consumers-final.json`。
...
CLAIM: R1统一了实际位图原语返回约定，升级与首次live职责分离。
  METHOD: read
  EVIDENCE: /root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/runtime/src/Heap/z/zLiveMap.inline.hpp:41；/root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/runtime/src/Heap/z/zLiveMap.inline.hpp:80；/root/cj_build/reference/jdk/src/hotspot/share/gc/z/zBitMap.inline.hpp:55

CLAIM: R2每代启动顺序已集中到对应成员，old selection在标记完成后。
  METHOD: read
  EVIDENCE: /root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/runtime/src/Heap/z/zGeneration.cpp:53；/root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/runtime/src/Heap/z/zGeneration.cpp:124；/root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/runtime/src/Heap/z/zRelocationSet.cpp:63
...
CLAIM: 已删除的水位/旧槽分配符号在候选的产品限定域无匹配。
  METHOD: read
  EVIDENCE: local:/root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/delivery-r2/source-checks-final.json:1（同一git grep表达式，候选rc=1）
CLAIM: 同一删除检索在基线有匹配，作为上述阴性结果的阳性对照。
  METHOD: read
  EVIDENCE: local:/root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/delivery-r2/source-checks-final.json:1（基线fb7d5282867aaa3b9d8b5df2f6691227ff3424ce，rc=0）
...
固定入口：`/root/cj_build/ops/bin/kkk2_build_two.sh /root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495 sym_cangjie_runtime_606_implement_r5674249495-final2`。摘要 `final2-build.log`，远端default/testable-configure.log和build.log。两构型独立并行、实际-j192；各configure/build rc=0，wall=53s/52s。观察静态回调只在MRT_TESTABLE_INTERNALS启用，宏关的产品mark、birth、selection和CHECK仍在。
...
UNIT_DEFAULT_RC=1
日志 kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/unit-default.log；533项，532通过，1失败。
UNIT_FILLER_RC=1
日志 kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/unit-filler.log；533项，532通过，1失败。CJRT_HEAP_FILLER=0，调用同一default的两枚ELF；创建时哈希见 `default-filler-identity.txt`。
UNIT_OHOS_RC=128
日志 kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/ohos-cwdfix/unit.log；独立OHOS-host configure/build rc=0/0，runner在归档缺.git的血缘读取退出128，未执行用例，不算OHOS通过。构建配方/产物hash/真实尝试记录均保留。
...
|green|727/1|0|仅既有P3失败|
|cut-strong|647/81|80|StrongClaimResult红；FinalizableClaimResult和内容/live控制通过|
|cut-final|714/14|13|FinalizableClaimResult红；StrongClaimResult和内容/live控制通过|
|cut-producer|710/18|17|single/striped字段follow目标红|
|cut-consumer|720/8|7|重复消费reached=2目标红|
|cut-mixed-selection|718/10|9|9项均命中owner CHECK|
|cut-combined|703/25|24|两端故障集合并集，字段follow与duplicate均红|
|restored|727/1|0|与green同一失败集|
...
CLAIM: 位图返回合同有精确阳性/阴性对照，内容与live控制没有随返回故障被破坏。
  METHOD: test
  EVIDENCE: kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/comparison/cut-strong/P1BitMap.StrongClaimResult.log；kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/comparison/cut-final/P1BitMap.FinalizableClaimResult.log；kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/comparison/restored/P1BitMap.ClaimContentsAndUpgradeControl.log
  N: 1
CLAIM: 基线producer调用与consumer结果各自影响目标字段闭包/重复消费断言，恢复回到基准。
  METHOD: test
  EVIDENCE: kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/comparison/cut-producer/YoungWeakClosure.SingleWorkerKeepsYoungReferentStrong.log；kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/comparison/cut-consumer/P1Mark.DuplicateAnyThreadStopsAtConsumer.log；kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/comparison/restored/P1Mark.DuplicateAnyThreadStopsAtConsumer.log
  N: 1
CLAIM: 混入另一代页被原owner CHECK拒绝，而恢复臂保留本包9项正常major结果。
  METHOD: control-arm
  EVIDENCE: kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/comparison/cut-mixed-selection/suite.log；kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/comparison/restored/suite.log；local:/root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/delivery-r2/final-evidence-summary.json
  N: 1
...
CLAIM: P1阶段的顺序回退只改变对应目标判词，恢复后目标全部通过，四臂均真实经过产品start入口。
  METHOD: control-arm
  EVIDENCE: kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/p1-four-arms/results.json；kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/p1-four-arms/green/0/run.log；kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/p1-four-arms/cut-old-order/0/run.log；kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/p1-four-arms/cut-young-order/0/run.log；kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/p1-four-arms/restored/0/run.log
  N: 3
CLAIM: 同一托管输入在P1阶段之后进入old字段trace并触发已登记#607的current资格守卫，未取得整程通过。
  METHOD: measure
  EVIDENCE: kkk2:/root/sym_cangjie_runtime_606_implement_r5674249495-final2/p1-four-arms/results.json（12条均含完整LOADFC记录、rc=-6）；local:/root/cj_build/cangjie_runtime_wt/sym_cangjie_runtime_606_implement_r5674249495/delivery-r2/final-evidence-summary.json
  N: 3
...
形式检查WIP版rc=0，记录form-check-wip.log；终态还会按最终交付头复查。临时bundle、隔离控制源码与旧构建对象已清理，cleanup.json/local-cleanup.json留痕；最终复验源、所有SO/ELF/日志保留。push结果与正式头在收尾追加；形式检查不代替独立Review。
