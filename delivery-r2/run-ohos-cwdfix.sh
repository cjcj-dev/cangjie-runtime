...
cd "$root" || exit 2
mkdir -p ohos-cwdfix
cd "$root/default/runtime" || exit 2
...
if [ "$rc" = 0 ]; then
 ...
 bash "$root/default/runtime/tests/gc_unit/run_standalone.sh" > $root/ohos-cwdfix/unit.log 2>&1
