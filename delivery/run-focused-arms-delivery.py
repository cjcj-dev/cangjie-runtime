...
r=Path('/root')/(lane+'-accept-delivery');dest=r/'focused';dest.mkdir(exist_ok=True)
tests=['P1Mark.AllocatingAndRelocatablePolicyMatrix','P1Mark.DuplicateAnyThreadStopsAtConsumer','P1Mark.ResurrectAndInactivePhasePolicies','P1Mark.PinnedReclaimedSlotIsNotAllocationSource','MarkAllocation.LargeHolderAndNewTargetAreImplicitlyLive','MarkAllocation.LargeHolderKeepsRootedExistingTargetLive','LargePageGeneration.ArrayRootKeepsYoungTargetLive','PinRoot.NativeHeldUnmarkedPinnedObjectSurvivesUntilRemove','LiveMap.PageBirthSequenceIsImplicitLive','MarkStackEntry.ObjectPoliciesAreIndependent','SharedSmallPage.AgeRefillAndRetirement','ForwardingPublicationProduct.CompactRegionDeadFromHasNoForwardingAndIsNotTlab']
elf=r/'unit-testable/cj_gc_unit';children=[]
(dest/'uptime-before.txt').write_bytes(subprocess.check_output(['uptime']))
for arm in ['green','cut-alloc','cut-retire','cut-consumer','cut-pinned','cut-shared','cut-inplace','restored']:
 product=Path('/root')/(lane+'-'+('accept' if arm in ['green','restored'] else arm)+'-delivery')/'testable/build/runtime-staging/lib/x86_64_Release'
 d=dest/arm;d.mkdir(exist_ok=True)
 (d/'identity.json').write_text(json.dumps({str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [r/'unit-testable/cj_gc_unit',r/'unit-testable/cj_gc_forwarding_publication_unit',product/'libcangjie-runtime.so',product/'libboundscheck.so']},indent=2))
 for test in tests:
  elf=r/('unit-testable/cj_gc_forwarding_publication_unit' if test.startswith('ForwardingPublicationProduct.') else 'unit-testable/cj_gc_unit')
