...
r=Path('/root')/(lane+'-accept-final');dest=r/'focused';dest.mkdir(exist_ok=True)
...
for arm in ['green','cut-alloc','cut-retire','cut-consumer','cut-pinned','cut-shared','cut-inplace','restored']:
 product=Path('/root')/(lane+'-'+('accept' if arm in ['green','restored'] else arm)+'-final')/'testable/build/runtime-staging/lib/x86_64_Release'
