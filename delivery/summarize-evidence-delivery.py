...
r=Path('/root/sym_cangjie_runtime_606_implement_r5673376405-accept-delivery')
rows={}
logs={x:r/f'unit-{x}.log' for x in ['default','testable','filler']}
logs.update({x:r/'control-arms'/x/'suite.log' for x in ['cut-alloc','cut-retire','cut-consumer','cut-pinned','cut-shared','cut-inplace','restored']})
...
for arm in ['cut-alloc','cut-retire','cut-consumer','cut-pinned','cut-shared','cut-inplace']:
...
rows['identities']={arm:json.loads((r/'focused'/arm/'identity.json').read_text()) for arm in ['green','cut-alloc','cut-retire','cut-consumer','cut-pinned','cut-shared','cut-inplace','restored']}
