...
ALWAYS_INLINE void ForwardBarrier::LogObject(BaseObject* obj) const { CJ_MCC_StickyLogLine(obj); }
...
void ForwardBarrier::WriteReference(BaseObject* obj, RefField<false>& field, BaseObject* ref) const
{
    LogObject(obj);
#if defined(CANGJIE_GC_DEBUG_EQUIPMENT)
    EmitSiteNoteWrite(EmitBarrierKind::Forward, obj, ref, true);
#endif
    IdleBarrier::WriteReference(obj, field, ref);
}

void ForwardBarrier::WriteStruct(BaseObject* obj, MAddress dst, size_t dstLen, MAddress src, size_t srcLen) const
{
    LogObject(obj);
...
#if defined(CANGJIE_GC_DEBUG_EQUIPMENT)
    EmitSiteNoteWrite(EmitBarrierKind::Forward, obj, newRef, true);
#endif
}
