...
// interiorfix/b4persist: source slot address for NotePush (static/stack/remset).
...
void WCollector::PushYoungObject(BaseObject* object, WorkStack& workStack, const char* origin) const
{
    ...
    if (!object->IsValidObject()) {
        ...
        B4PersistProbe::FlushSummary("invalid-minor-root");
                                     ...
                                     bool fullYoungScan, MinorSlotSet* consumedOut, DiffPathRemsetStats* statsOut)
{
    ...
    for (MAddress slot : rememberedSlots) {
        ...
        B4PersistProbe::NoteRemsetConsume(slot, target);
...
void WCollector::DoYoungGarbageCollection()
{
    ...
    B4PersistProbe::FlushSummary("post-minor-trace");
