...
public:
                                         ...
                                         const ForwardingProvenance& provenance) const override
    {
        ...
        // ZRelocate::forward_object (zRelocate.cpp:412-415) returns find() whenever
        // the table has an answer. Identity (to==from) is a legal receipt for a
        // kept or in-place survivor; it is not a from-address fallback.
        // WaitRoutedTipReady only returns a non-null pointer from an ArmedHit or
        // a published request receipt, so resolved==obj is that identity case.
        if (resolved != nullptr) {
            return resolved;
        }
        CHECK_DETAIL(false,
