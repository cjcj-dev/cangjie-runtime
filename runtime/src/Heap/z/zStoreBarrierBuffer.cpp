...
void StoreBarrierBuffer::install_base_pointers_inner()
{
    for (size_t i = current; i < kStoreBarrierBufferLength; ++i) {
        ...
        const Generation gen =
            (generation == &Heap::GetHeap().GetCollector().GetZGeneration(GCCycleGeneration::YOUNG))
                ? Generation::Young
                : Generation::Old;
...
{
    return Heap::GetHeap().GetCollector().GetZGeneration(GCCycleGeneration::OLD).IsPhaseMark();
}
