...
void TracingCollector::TracingImpl(WorkStack& workStack, WorkStack& foreignRootsSet)
{
    ...
    BuildExportForeignAbiView(foreignRootsSet);
    VerifyMarkingStacks::VerifyEmpty(VerifyMarkingStacks::MarkingGeneration::MAJOR,
                                     VerifyMarkingStacks::MarkingBoundary::JOIN,
...
{
    // BuildExportForeignAbiView publishes export-keyed foreign values after
    // FollowWork has closed ordinary liveness. Resolve and rekey that carrier
    // before its first mark-bit read. The producer lock remains the owner
    // boundary; no field obligation is introduced for these root referents.
    {
...
void TracingCollector::BuildExportForeignAbiView(RootSet& exportRoots)
{
    // ZGC mark bits only express liveness (zMark.cpp:667-670, zMark.inline.hpp:61-87).
    // Cangjie foreign ABI needs export-root → foreign-proxy ownership. Produce that
    // read-only view once after FollowWork, keyed by the export root, from mark
    // results plus the export root set. Do not mix the relation into mark bits.
    MarkLiveCache liveCache(1);
    while (!exportRoots.empty()) {
        const MarkStackEntry rootEntry = exportRoots.back();
        exportRoots.pop_back();
        if (rootEntry.partialArray()) {
            continue;
        }
        BaseObject* exportObj = rootEntry.object();
        if (exportObj == nullptr) {
            continue;
        }
        (void)MarkEntryObject(exportObj, MarkStackEntry::MarkAndFollow(exportObj, rootEntry.finalizable()),
                              &liveCache);
        WorkStack workStack;
        workStack.push_back(MarkStackEntry::FollowOnly(exportObj, rootEntry.finalizable()));
        std::list<BaseObject*> externObjs;
        BaseObject* obj = nullptr;
        bool finalizable = rootEntry.finalizable();
        auto visit = [&workStack, &obj, this, &externObjs, &liveCache, &finalizable](MAddress slot) {
                ...
                if (is_mark_good(oldField)) {
                    ...
                    if (!Collector::MarkGoodHeapGate("BuildExportForeignAbiView", targetObj)) {
                        return;
                    }
                    ...
                    } else if (!MarkEntryObject(targetObj, MarkStackEntry::MarkAndFollow(targetObj, finalizable),
                                                &liveCache)) {
                        workStack.push_back(MarkStackEntry::FollowOnly(targetObj, finalizable));
                    }
                    return;
                ...
                if (oldField.GetFieldValue() == newField.GetFieldValue()) {
                    ...
                    DLOG(TRACE, "trace obj %p ref@%p: %#zx => %#zx->%p<%p>(%zu)", obj, &field,
                ...
                } else if (!MarkEntryObject(latest, MarkStackEntry::MarkAndFollow(latest, finalizable),
                                            &liveCache)) {
                    workStack.push_back(MarkStackEntry::FollowOnly(latest, finalizable));
                }
        };
        ...
        discoveredExternObjects[exportObj] = std::move(externObjs);
    }
    liveCache.Flush();
}
