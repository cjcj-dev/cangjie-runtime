...
void NoteRetiredReader(MAddress from, MAddress to)
{
    ...
    if (sample <= 256) {
        ...
        const unsigned holderState = holderRegion == nullptr
            ? UINT32_MAX
            : static_cast<unsigned>(context.holder->GetStateWord().GetStateCode());
        ...
        LOG(RTLOG_ERROR,
            ...
            "slotHeap=%u slotRtype=%u slotYoung=%u holder=%p holderHeap=%u holderState=%u "
...
ForwardingTable::ReaderScope::ReaderScope(ReaderKind kind, const void* slot, const BaseObject* holder,
                                         const void* caller, const void* origin)
...
{
    g_readerContext = ReaderContext{ previousKind, previousSlot, previousHolder, previousCaller, previousOrigin };
}
