                          ...
                          uint8_t phase, bool probeToVersion)
{
    ...
    const unsigned routeState = region == nullptr ? 7u : static_cast<unsigned>(region->GetPageRelocate()) & 0x7u;
...
void NoteStaleGuardFired(bool zeroHeader, bool resolved, BaseObject* target, BaseObject* holder, const void* slot)
{
    ...
    if ((n & (n - 1)) == 0 || logUnusable) {
        ...
        const unsigned route = region == nullptr ? 9u : static_cast<unsigned>(region->GetPageRelocate());
