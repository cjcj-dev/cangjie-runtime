...
// Compile-time gate. Census (Z-2) does not need the auditor; A-V3 audit
// arm flips this true in a later commit. getenv MRT_GCV2_* is pinned-off.
constexpr bool kFysAudit = false;
...
bool RetainedWouldDrop(BaseObject* holder, RegionInfo* holderRegion)
{
    ...
    if (holderRegion->IsLargeRegion()) {
        ...
        MarkView<Generation::Old> retainedView = holderRegion->GetMarkView<Generation::Old>();
    ...
    if (holderRegion->HasRetainedMarkWords()) {
        return !holderRegion->RetainedMarkWordsSay(holderOffset);
    }
    LiveInfo* retainedLiveInfo = holderRegion->GetRetainedLiveInfo();
    ...
    MarkView<Generation::Old> retainedView = holderRegion->GetMarkView<Generation::Old>();
    return !holderRegion->IsSurvivedObject(retainedView, retainedLiveInfo, holderOffset);
}
...
void CensusPrePinned(size_t minorRunIndex)
{
    ...
    Heap::GetHeap().ForEachObj(
        [&remsetSnap](BaseObject* holder) {
            ...
            ++g_c.holdersScanned;
            const bool holderMarked = holderRegion->IsYoungRegion()
                ? holderRegion->IsMarkedObject(holderRegion->GetMarkView<Generation::Young>(), holder)
                : holderRegion->IsMarkedObject(holderRegion->GetMarkView<Generation::Old>(), holder);
            if (holderMarked) {
                ++g_c.holderMarked;
            } else {
                ++g_c.holderUnmarked;
