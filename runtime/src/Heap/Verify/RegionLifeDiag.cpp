...
namespace {
thread_local uint16_t gNextFreePath = PATH_COLLECT_GENERIC;
}
...
void SetNextFreePath(uint16_t path) { gNextFreePath = path; }
uint16_t TakeNextFreePath()
{
    const uint16_t path = gNextFreePath;
    gNextFreePath = PATH_COLLECT_GENERIC;
    return path;
}
