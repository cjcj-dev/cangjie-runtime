                       ...
                       const std::unordered_set<BaseObject*>* rootReachableHolders)
{
    ...
    if (holder != nullptr && holderRegion != nullptr && !holderRegion->IsFreeRegion() &&
        !holderRegion->IsGarbageRegion()) {
        holderMarked = static_cast<int>(holderRegion->IsMarkedObject(holder));
        holderSurvived = static_cast<int>(Heap::GetHeap().IsSurvivedObject(holder));
    }
    ...
    VLOG(REPORT,
         ...
         "holderMarked=%d holderSurvived=%d holderReachabilityKnown=%u holderReachable=%u "
         "failure=%zu max=%zu env=MRT_GCV2_VERIFY_HEAP=1",
         ...
         holderType, holderMarked, holderSurvived, static_cast<unsigned>(reachabilityKnown),
         static_cast<unsigned>(holderReachable), stats.failures, maxFailures);
}
