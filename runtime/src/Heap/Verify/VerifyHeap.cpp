...
//   1) null / misaligned → fail
//   2) tip ∈ heap address range → fail (defect D signature: tip in heap anonymous)
//   3) !IsVaildType() → fail (type byte ≥ TYPE_KIND_MAX)
//   4) TypeInfoManager::ContainsAddress(tip) → strongest online positive
//   5) else non-heap + valid type → accept (static TypeInfo in load module)
// Anchor intent: HotSpot oop verify + our gcvroot tipRegion==HEAP reject.
bool CheckTypeInfoRegion(TypeInfo* tip, HeapVerifyStats& stats, const char*& reason)
{
    if (tip == nullptr) {
        ++stats.h2NullTip;
...
bool CheckObjectH1H2(BaseObject* obj, HeapVerifyStats& stats, const char*& reason)
{
    if (obj == nullptr) {
        reason = "null-object";
        return false;
...
void ReportFailure(HeapVerifyStats& stats, size_t maxFailures, const char* reason, BaseObject* obj,
...
void VerifyHeapObjects(const char* point)
{
    ...
    Heap::GetHeap().ForEachObj(
        [&stats, maxFailures](BaseObject* obj) {
            ...
            if (region == nullptr || region->IsFreeRegion() || region->IsGarbageRegion()) {
                ++stats.h4BadRegion;
                ReportFailure(stats, maxFailures, "bad-region", obj, nullptr, typeByte);
                return;
            }
            ...
            if (!CheckObjectH1H2(obj, stats, reason)) {
                ...
                ReportFailure(stats, maxFailures, reason, obj, nullptr, typeByte);
                return;
            }
            ...
            obj->ForEachRefField([&stats, maxFailures, obj](RefField<>& field) {
                ...
                if (!CheckObjectH1H2(target, stats, tReason)) {
                    ++stats.h3BadRef;
                    int tByte = -1;
                    ...
                    ReportFailure(stats, maxFailures, tReason, target, obj, tByte);
                }
            });
        },
        false);
    ...
    VLOG(REPORT,
         ...
         "objects=%zu failures=%zu truncated=%zu "
         ...
         "samples=[%p,%p,%p,%p]",
         point == nullptr ? "?" : point, invoke, stats.objectsScanned, stats.failures, stats.truncated,
         stats.h1InvalidObject, stats.h2NullTip, stats.h2MisalignedTip, stats.h2TipInHeap, stats.h2InvalidTypeKind,
         stats.h2TipInTim, stats.h2TipNonHeapOk, stats.h3BadRef, stats.h4BadRegion,
         static_cast<unsigned long long>(stats.costNs), maxFailures, stats.samples[0], stats.samples[1],
         stats.samples[2], stats.samples[3]);
