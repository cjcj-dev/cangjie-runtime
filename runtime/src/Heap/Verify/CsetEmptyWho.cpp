...
std::atomic<size_t> g_derivedSeen{ 0 };
...
std::atomic<size_t> g_clsDerived{ 0 };
...
void BumpClass(Who w)
{
    switch (w) {
        ...
        case Who::Stack:
            ...
            break;
        case Who::Derived:
            g_clsDerived.fetch_add(1, std::memory_order_relaxed);
            break;
        case Who::ExtraRoot:
...
void VisitRootSlot(RootSlot& root, Who cls, std::atomic<size_t>& counter)
{
    ...
    if (Heap::IsHeapAddress(target) &&
        !Collector::PlausibleManagedObjectGate("CsetEmptyWho.interior", target)) {
...
void ClassifyCycle()
{
    ...
    // VisitExceptionRoots. Do not use VisitMutatorRoots (drops derived).
    MutatorManager::Instance().VisitAllMutators([](Mutator& mutator) {
        RootVisitor stackVisitor = [](RootSlot& root) { VisitRootSlot(root, Who::Stack, g_stackSeen); };
        DerivedPtrVisitor derivedVisitor = [](BasePtrType basePtr, DerivedSlot& derivedPtr) {
            (void)derivedPtr;
            if (is_null(basePtr)) {
                return;
            }
            BaseObject* base = to_object(safe(uncolor_bits(to_zpointer(raw(basePtr)))));
            g_derivedSeen.fetch_add(1, std::memory_order_relaxed);
            HitTarget(base, Who::Derived);
        };
...
void Report(const char* tag)
{
    std::fprintf(stderr,
                 ...
                 "derivedSeen=%zu extraSeen=%zu "
                 "page STATIC=%zu STACK=%zu DERIVED=%zu EXTRA=%zu YOUNG=%zu OLD_UNMARKED=%zu "
                 "OLD_MARKED=%zu NONE=%zu walkNs=%zu\n",
                 tag != nullptr ? tag : "?",
                 ...
                 g_derivedSeen.load(std::memory_order_relaxed),
                 ...
                 g_clsDerived.load(std::memory_order_relaxed),
