...
bool GateOn()
{
    static const bool on = []() {
        ...
        if (EnvIsOne("MRT_GCV2_GARBREGION")) {
            return true;
        }
        return DiagGate::TokenOn("garbregion");
    }();
    return on;
}
...
std::atomic<size_t> g_censusRegionsHit{ 0 };
...
void CensusBeforeForward(const char* where)
{
    ...
    g_censusRegionsHit.store(g_tabUsed.load(std::memory_order_relaxed), std::memory_order_relaxed);
    std::fprintf(stderr,
...
void NoteCollectEnter(RegionInfo* region)
{
    ...
    if (loud) {
        std::fprintf(stderr,
                     ...
                     "young=%u large=%u live=%llu auth=%u knownEmpty=%u neverExamined=%u "
                     ...
                     static_cast<unsigned>(region->GetRouteState()), static_cast<unsigned>(young),
                     static_cast<unsigned>(large), static_cast<unsigned long long>(liveBytes),
                     static_cast<unsigned>(auth), static_cast<unsigned>(knownEmpty),
                     static_cast<unsigned>(neverExamined), liveInfo, static_cast<unsigned long long>(faceEp),
                     static_cast<unsigned long long>(snapEp), mb, cls, slots, markedH, unmarkedH, youngH, oldH,
                     static_cast<unsigned>(row != nullptr), g_gcCount.load(std::memory_order_relaxed), g_lastWhere);
        std::fflush(stderr);
    }
}
...
void NoteF3Join(RegionInfo* latestRegion, BaseObject* latest, const char* reason)
{
    ...
    if (slots > 0 || slog < kSampleCap) {
        std::fprintf(stderr,
                     "[GCV2][garbregion][f3] n=%zu reason=%s region=%p latest=%p type=%u "
                     "live=%llu auth=%u knownEmpty=%u liveinfo=%p "
...
void Report(const char* point)
{
    ...
    std::fprintf(stderr,
                 ...
                 g_censusRegionsHit.load(std::memory_order_relaxed), g_tabSat.load(std::memory_order_relaxed),
