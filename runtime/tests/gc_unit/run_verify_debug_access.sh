...
/usr/bin/grep -Eq ' CJ_MCC_LoadBarrierOnOopFieldPreloaded(@[^[:space:]]+)?$' "$OUT/product-defined.txt"
/usr/bin/grep -Eq ' CJ_MCC_LoadBarrierOnOopFieldPreloaded(@[^[:space:]]+)?$' "$OUT/test-imports.txt"
if /usr/bin/grep -Eq ' CJ_MCC_LoadBarrierOnOopFieldPreloaded(@[^[:space:]]+)?$' "$OUT/test-defined.txt"; then exit 79; fi
