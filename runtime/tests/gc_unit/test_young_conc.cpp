...
GC_TEST(YoungConc, LateEdgeFollowReceiptSurvivesSatbHandoff)
{
    ...
    SatbBuffer::Young().GetRetiredEntries([&](BaseObject* object, bool follow) {
        sawObject = object;
        sawFollow = follow;
    });
...
GC_TEST(YoungConc, PublishYoungAllocBlackRetiresFollowReceipt)
{
    ...
    bool follow = false;
    SatbBuffer::Young().GetRetiredEntries([&](BaseObject* entry, bool shouldFollow) {
        object = entry;
        follow = shouldFollow;
    });
...
GC_TEST(YoungConc, LegacyTerminationMissesUnfullSatbNode)
{
    ...
    SatbBuffer::Young().GetRetiredObjects(markWork);
