...
CUTS = {
    ...
    'claim': ('runtime/src/Heap/Collector/zRelocate.cpp',
              '    if (!owner || (!claimed && !owner->claim())) return;',
              '    if (!owner) return;', 1),
    'reader': ('runtime/src/Heap/Collector/zRelocate.cpp',
               '        owner->in_place_relocation_claim_page();',
               '        (void)owner;', 2),
    'find': ('runtime/src/Heap/Collector/zRelocate.cpp',
             '    return reinterpret_cast<BaseObject*>(owner->find(from));',
             '    return obj;', 1),
    'done': ('runtime/src/Heap/Collector/zRelocate.cpp',
             '    ForwardRegion<G>(region);\n#if defined(MRT_TESTABLE_INTERNALS)',
             '    ForwardRegion<G>(region);\n    owner->mark_done();\n#if defined(MRT_TESTABLE_INTERNALS)', 1),
}
CUTS['reader_partial'] = ('runtime/src/Heap/Collector/zRelocate.cpp',
