...
extern "C" int p2FieldBarrierExercise()
{
    ...
    ZBarrier::testFieldMarkResult = [&](ZBarrier::FieldMarkKind kind, RefField<>& field,
        ...
        if (kind == ZBarrier::FieldMarkKind::Finalizable && finalHolder != nullptr) {
            Expect((collector.GetCycleSnapshot(GCCycleGeneration::OLD).phase == ZGenerationPhase::Mark ||
                    collector.GetCycleSnapshot(GCCycleGeneration::OLD).phase == ZGenerationPhase::MarkComplete), "finalizable_follow_during_mark");
