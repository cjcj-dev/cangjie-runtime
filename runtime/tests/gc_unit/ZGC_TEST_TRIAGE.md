...
| `test_zBitField.cpp` | 7 | `BitField<T>` in `RegionInfo.h` | Port all seven encode/decode round trips through `SetAtomicValue` / `GetAtomicValue`, including wide and pointer-compressed values | PORTED; exact seven assertion sites in `test_zBitField.cpp`; wide-mask UB fixed in product `FieldMask`; deliberately red, then green in both states |
