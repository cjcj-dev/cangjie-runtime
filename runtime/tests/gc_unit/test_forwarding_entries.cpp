...
GC_TEST(ZForwardingEntries, ConcurrentSameKeyReturnsInitializedWinner)
{
    ...
    a.join();
    b.join();
    const bool sameWinner = returned[0] == returned[1];
    std::fprintf(stderr, "P1_WINNER target_assertion executed=1 matched=%d a=%#zx b=%#zx\n",
                 sameWinner, returned[0], returned[1]);
