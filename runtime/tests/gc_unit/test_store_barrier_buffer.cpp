...
// Old null slots must be store-bad (remembered 11) so the compiler store-bad
// test takes the buffer path. store_good(null) is the fast-path color.
GC_TEST(OldToYoung1102, PromotedNullIsStoreBadAndSlowStoreRemembersSlot)
{
    ...
    std::fprintf(stderr, "OLD_TO_YOUNG_1102_PROMOTED raw=%zx bad=%d store_good_null_not_bad=%d\n",
                 raw(promoted), promotedBad, storeGoodNotBad);
    GC_EXPECT_TRUE(promotedBad && storeGoodNotBad && promoted == color_null());
