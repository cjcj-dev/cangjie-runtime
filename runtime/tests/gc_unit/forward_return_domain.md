...
The baseline uses a full `--clean-first` product build. Every arm disables the
...
`cut_identity` restores the rejected identity predicate in the product SO;
`cut_copy_return` rejects only non-identity. Negative cuts bypass the matching
`WaitRoutedTipReady::permanentHole` failure with an invented from-address.
There is no accept-arm source replacement and no old-guard baseline assumption.

| Domain | Lookup / actual product result | done=0 / done=1 | Targeted cut |
|---|---|---|---|
| Identity | Current active ArmedHit, to==from: return receipt | Both legal | Reject identity at outer guard |
...
` consumer=WCollector::WaitRoutedTipReady ` and SIGABRT together. They cannot
reach the outer guard's CHECK while the inner failure works. The outer guard
is already non-null-only, so that predicate is not a negative-case mutation.
The unavailable fixture covers a closed/unavailable publication, not every
possible interleaving of a still-in-flight publisher.
...
5ee81066e5493eea3a198fe9dba701dc7dd42023, in
