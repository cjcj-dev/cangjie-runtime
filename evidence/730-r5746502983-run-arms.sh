...
run_arm() (
  arm=$1; config=$2
  arm_source=$root-$arm
  if [ "$arm" = green ] || [ "$arm" = restored ]; then arm_source=$source_root; fi
  lib=$arm_source/$config/build/runtime-staging/lib/x86_64_Release
  elf=$source_root/unit-$config/cj_gc_unit
  dest=$out/$arm-$config
  mkdir -p "$dest"
  ...
  tests=(ObjectAllocatorPaths.ManagedSizeRouting ObjectAllocatorPaths.TLABsShareSmallPage ObjectAllocatorPaths.FastMediumConsumesCachedActualSize ObjectAllocatorPaths.ManagedFastMediumConsumesCachedPage ObjectAllocatorPaths.NonBlockingCapacityDoesNotStartCollection TLABUsage.BoundsAndDemand AllocationStall.WaiterBlocksInSaferegion SegmentedArrayInit.SmallReferenceArrayKeepsFastPath SegmentedArrayInit.LargeReferenceArrayPublishesClearedPayload SegmentedArrayInit.LargePrimitiveArrayUsesSegmentedClearing)
  if [ "$config" = testable ]; then
    tests+=(SegmentedArrayInit.EpochFlipRestartsAndRewritesPublishedBlock SegmentedArrayInit.YoungGcRepairsIncompleteArrayRoot SegmentedArrayInit.TwoGcReferenceInitializationRestartsOnlyOnce SegmentedArrayInit.TwoGcPrimitiveInitializationDoesNotRestart SegmentedArrayInit.PrimitivePayloadSurvivesFullGcWindow)
  fi
...
for config in default testable; do
  (
    ...
    count=0
    for arm in green tlab-producer tlab-consumer small medium-producer medium-consumer large segment-producer segment-consumer stall stall-producer restored; do
      run_arm "$arm" "$config" &
      count=$((count+1))
