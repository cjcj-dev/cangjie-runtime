...
cd "$root"
start=$SECONDS
trap 'rc=$?; echo "$rc" > evidence/std-v4.rc; echo "$((SECONDS-start))" > evidence/std-v4.wall; uptime > evidence/std-v4-after.txt' EXIT
uptime > evidence/std-v4-before.txt
cp "$lane/frontend-r2/cjcj-stage1-r3" target/bin/cjcj-stage1
...
sha256sum llvm-build/bin/llc llvm-build/bin/opt target/runtime/lib/linux_x86_64_cjnative/libcangjie-runtime.so target/runtime/lib/linux_x86_64_cjnative/libboundscheck.so > evidence/std-v4-inputs.sha256
