#!/bin/bash
ulimit -c 0
R=/root/sym_cangjie_runtime_740_implement_r5747608277-ohos
SRC=/root/sym_cangjie_runtime_740_implement_r5747608277-green/default/runtime
mkdir -p "$R"
export CCACHE_DIR=/root/.ccache CCACHE_BASEDIR=/root/sym_cangjie_runtime_740_implement_r5747608277-green/default CCACHE_NOHASHDIR=1 GC_UNIT_GATE_SKIP=1
...
if [[ $rc = 0 ]]; then
 ...
 env MRT_GC_UNIT_OHOS_HOST=1 GC_UNIT_OUT="$R/unit" GCV2_RUNTIME_LIB_DIR="$LIB" GCV2_RUNTIME_OUTPUT_ROOT="$R/build/runtime-staging" bash "$SRC/tests/gc_unit/run_standalone.sh" > "$R/run.log" 2>&1
 rc=$?; echo "$rc" > "$R/run.rc"
fi
echo "$rc" > "$R/arm.rc"
