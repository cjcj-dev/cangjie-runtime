...
cd "$root"
start=$SECONDS
trap 'rc=$?; echo "$rc" > evidence/std-v5.rc; echo "$((SECONDS-start))" > evidence/std-v5.wall; uptime > evidence/std-v5-after.txt' EXIT
uptime > evidence/std-v5-before.txt
...
sha256sum llvm-build/bin/llc llvm-build/bin/opt target/runtime/lib/linux_x86_64_cjnative/libcangjie-runtime.so target/runtime/lib/linux_x86_64_cjnative/libboundscheck.so > evidence/std-v5-inputs.sha256
