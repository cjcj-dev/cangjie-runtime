...
readonly GREEN=/root/sodepot/impl_gcunit3/r3-final-green
readonly ROOT_OUT="$REPO/evidence/impl_gcunit3/r3-product-arms"
readonly BACKUP=/root/impl_gcunit3-r3-product-backup
...
run_arm()
{
    ...
    local depot="/root/sodepot/impl_gcunit3/r3-$name"
    mkdir -p "$out" "$depot"
    restore_sources
...
run_arm cut-mutator-root-walk /root/r3-cut-mutator-native.patch || exit $?
run_arm cut-watermark-drain /root/r3-cut-watermark-native.patch || exit $?
run_arm cut-fix-minor-slot /root/r3-cut-minor-relocate.patch || exit $?
run_arm cut-iterator-skip /root/r3-cut-iterator-skip.patch || exit $?
...
readonly RESTORED=/root/sodepot/impl_gcunit3/r3-restored
...
trap - EXIT
touch "$ROOT_OUT/R3_PRODUCT_ARMS_DONE"
