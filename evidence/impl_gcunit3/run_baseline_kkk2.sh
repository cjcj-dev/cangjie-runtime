...
readonly SOURCE_REPO=/root/repos/cangjie_runtime
...
if [[ -e "$REPO" ]]; then
    echo "refusing to reuse existing path: $REPO" >&2
    exit 73
fi

git clone --shared --no-checkout "$SOURCE_REPO" "$REPO" || exit $?
git -C "$REPO" checkout --detach "$BASELINE" || exit $?
...

{
    ...
    echo "CONFIGURATION=MRT_GC_UNIT_TESTS=ON,MRT_TESTABLE_INTERNALS=ON"
    echo "CORES=$CORES"
    taskset -pc $$
} >"$EVIDENCE/preflight.log" 2>&1
...
configure_rc=$?
printf '%s\n' "$configure_rc" >"$EVIDENCE/configure.rc"
