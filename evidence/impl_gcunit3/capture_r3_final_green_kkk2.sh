...
readonly PRODUCT=/root/sodepot/impl_gcunit3/r3-green
readonly DEPOT=/root/sodepot/impl_gcunit3/r3-final-green
readonly TEST_OUT="$REPO/runtime/tests/gc_unit/build_impl_gcunit3_r3_final"
readonly OUT="$REPO/evidence/impl_gcunit3/r3-final-green"
...
mkdir -p "$DEPOT" "$TEST_OUT" "$OUT" "$OUT/managed"
{
    ...
    uptime
    taskset -pc $$
    sha256sum "$PRODUCT/libcangjie-runtime.so" "$PRODUCT/libboundscheck.so" \
    ...
    stat -c '%y %n' "$REPO/runtime/src/Heap/Collector/Mark.cpp" \
        "$REPO/runtime/src/ObjectModel/MArray.cpp" "$REPO/runtime/src/ObjectModel/MArray.h" \
        "$REPO/runtime/tests/gc_unit/test_m0_exit.cpp" \
        "$REPO/runtime/tests/gc_unit/test_segmented_array_init.cpp"
