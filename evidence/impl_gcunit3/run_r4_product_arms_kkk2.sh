...
readonly ROOT_OUT="$REPO/evidence/impl_gcunit3/r4-product-arms"
readonly BACKUP=/root/impl_gcunit3-r4-product-backup
...
run_tests()
{
    ...
    for test_name in "${tests[@]}"; do
        ...
        set +e
        LD_LIBRARY_PATH="$libdir" taskset -c "$CORES" "$libdir/cj_gc_unit" \
...

{
    ...
    uptime
    taskset -pc $$
    sha256sum "$MUTATOR" "$RELOCATE" "$BASE_OBJECT" "$MARRAY" "$GREEN/cj_gc_unit"
} >"$ROOT_OUT/preflight.log" 2>&1

run_arm cut-mutator-root-walk /root/r4-cut-mutator-native.patch || exit $?
run_arm cut-watermark-drain /root/r4-cut-watermark-native.patch || exit $?
run_arm cut-fix-minor-slot /root/r4-cut-minor-relocate.patch || exit $?
run_arm cut-iterator-skip /root/r4-cut-iterator-skip.patch || exit $?

restore_sources
sha256sum "$MUTATOR" "$RELOCATE" "$BASE_OBJECT" "$MARRAY" >"$ROOT_OUT/restored-source.sha256"
{
    for pair in \
        "$BACKUP/Mutator.cpp:$MUTATOR" \
        "$BACKUP/Relocate.cpp:$RELOCATE" \
        "$BACKUP/BaseObject.cpp:$BASE_OBJECT" \
        "$BACKUP/MArray.cpp:$MARRAY"; do
        before=${pair%%:*}
        after=${pair#*:}
        cmp -s "$before" "$after"
        printf '%s\t%s\n' "$after" "$?"
    done
} >"$ROOT_OUT/source-restore-cmp.tsv"

readonly RESTORED=/root/sodepot/impl_gcunit3/r4-restored
...
trap - EXIT
touch "$ROOT_OUT/R4_PRODUCT_ARMS_DONE"
