...
TESTROOT=$BASE-bind
ARTIFACTS=$TESTROOT/movable-arrays
OUT=$BASE/cd925-cuts
...
run() {
 local arm=$1 entry=$2
 local lib=$BASE-cut-$arm/testable/build/runtime-staging/lib/x86_64_Release
 [[ $arm = green || $arm = restored ]] && lib=/root/diff_cd925f409158/testable/build/runtime-staging/lib/x86_64_Release
 ...
 bash "$TESTROOT/testable/runtime/tests/gc_unit/run_p2_field_barrier.sh" > "$o/run.log" 2>&1
 local rc=$?
 echo $rc > "$o/run.rc"
...
for arm in green entry consumer partial final strong nonmajor restored; do
 for entry in p2FieldBarrierExercise p2SlowFieldInputExercise p2MinorDuringOldMarkExercise p2ArrayFieldExercise; do
  (run "$arm" "$entry") &
 done
done
wait
