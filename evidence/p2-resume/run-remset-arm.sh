...
case "$arm" in
 ...
 producer) P=/root/sym_cangjie_runtime_607_implement_r5684610492-producer2/testable; cores=120-127;;
 consumer) P=/root/sym_cangjie_runtime_607_implement_r5684610492-consumer2/testable; cores=128-135;;
 restored) P=/root/sym_cangjie_runtime_607_implement_r5684610492-restored2/testable; cores=112-119;;
 *) exit 2;;
esac
SO="$P/build/runtime-staging/lib/x86_64_Release"
SDK=/root/sym_cangjie_runtime_607_implement_r5684610492-build3/sdk
O="$R/qualified-remset/$arm"
mkdir -p "$O"
...
start=$(date +%s%N)
P2_MINOR_ONLY=1 LD_DEBUG=libs LD_LIBRARY_PATH="$SO:$A:$SDK/runtime/lib/linux_x86_64_cjnative" cjGCInterval=3600s \
...
import json,sys
p=Path(sys.argv[1]); lines=(p/'run.log').read_text().splitlines(); d={'rc':int((p/'run.rc').read_text()),'assertions':[x for x in lines if x.startswith('P2_ASSERT ')], 'result':[x for x in lines if x.startswith('P2_RESULT')]}; (p/'result.json').write_text(json.dumps(d,indent=2)+'\n');print(p.name,d['rc'],len(d['assertions']));print('\n'.join(x for x in d['assertions'] if x.endswith('FAIL') or 'root_control' in x));print(d['result'])
PY
