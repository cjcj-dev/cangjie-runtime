...
case "$arm" in
 green) P="$R/testable";;
 restored) P=/root/$L-restored8/testable;;
 *) P=/root/$L-$arm/testable;;
esac
case "$scenario" in
 slow) A="$R/slow-artifacts"; INPUT=();;
 full) A="$R/field-artifacts"; INPUT=(P2_FINALIZABLE=1);;
 minor) A="$R/field-artifacts"; INPUT=(P2_MINOR_ONLY=1);;
 array) A="$R/array-artifacts"; INPUT=();;
 array-final) A="$R/array-artifacts"; INPUT=(P2_ARRAY_FINALIZABLE=1);;
 struct) A="$R/array-artifacts"; INPUT=(P2_STRUCT_ARRAY=1);;
 struct-final) A="$R/array-artifacts"; INPUT=(P2_STRUCT_ARRAY=1 P2_ARRAY_FINALIZABLE=1);;
 closure) A=/root/$L-build7/closure-artifacts; INPUT=();;
 registration) A=/root/$L-build6/registration3; INPUT=();;
 *) exit 2;;
esac
SO="$P/build/runtime-staging/lib/x86_64_Release"
SDK=/root/$L-build3/sdk
O="$R/qualified-$scenario/$arm"
mkdir -p "$O"
