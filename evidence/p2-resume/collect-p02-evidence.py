...
for a in ('default','filler','testable'):
 o=r/('unit-p02-done-'+a);s=(o/'run.log').read_text();d['units'][a]={'rc':int((o/'run.rc').read_text()),'tail':s.splitlines()[-15:],'files':{q.name:q.read_text() for q in o.iterdir() if q.suffix in ('.sha256','.txt','.rc')}}
...
for name,p in [('product',r/'testable/build/runtime-staging/lib/x86_64_Release/libcangjie-runtime.so'),('test',r/'slow2/libp2_field_barrier.so')]:
