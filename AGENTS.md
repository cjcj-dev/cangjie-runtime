...
⭐⭐⭐ `EVIDENCE=` 除绝对路径外必须带主机前缀：本机写 `local:/root/...`，kkk2 写 `kkk2:/root/...`。双机拓扑中绝对路径不是唯一标识；本机查不到时，审查方应先经 `bash /root/cj_build/tools/box.sh kkk2 'ls -ld <路径>'` 到 kkk2 复核，不能直接判定证据不存在。
